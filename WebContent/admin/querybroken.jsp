...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>设备报废管理 <span class="c-gray en">&gt;</span> 设备报废查询 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="7">设备报废查询</th>
				</tr>
				<tr class="text-c">
					<th class="center">设备</th>
					<th class="center">报废原因</th>
					<th class="center">处理结果</th>
					<th class="center">损失</th>
			...
			<c:forEach items="${brokenList}" var="broken">
				<tr class="text-c">
					<td class="center">${broken.equipname}</td>
					<td class="center">${broken.reason}</td>
					<td class="center">${broken.resultx}</td>
					<td class="center">${broken.money}</td>
					<td class="center">${broken.addtime}</td>
					<td class="center">${broken.memo}</td>
				</tr>
			</c:forEach>
		</table>
		...
		<div class="text-c">
			<form action="broken/queryBrokenByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						...
						<option value="reason">按报废原因查询</option>
						<option value="resultx">按处理结果查询</option>
						<option value="money">按损失查询</option>
